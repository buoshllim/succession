...
> **조사 규칙:** "이사"는 모음(ㅏ)으로 끝나므로 항상 `가/는/를/로` 사용. `이가/은/을/으로` 금지.
> - ✅ "Jensen 이사가", "Buffett 이사는", "Munger 이사를"
> - ❌ "Jensen 이사이가", "Buffett 이사은", "Munger 이사이"
...
**금지:**
- 준비도 점수 수식 노출 (`raw 0.73 → 0.865` 등 내부 계산 그대로 기재 금지)
- 이사별 가중치 수치 열거 (`Vision(0.25)+Buffett(0.17)...` 형태 금지)
- 보고서식 항목 나열 (기계적 요약 금지)

**필수 구조:**
```
본 이사회는 {N}라운드 심의를 마치고 다음과 같이 의결한다.

{심의 과정에서 결정적으로 작용한 논거·전환점을 서술 — 수치 없이 판단 중심으로}

**이사회는 {1순위 후보자}를 {포지션} 1순위 후보자로 의결한다.**

단, 선임 전 아래 조건 이행을 전제로 한다.  ← 조건 있을 경우만
- {조건 1}
- {조건 2}

{2순위 후보자 처우 — 재심의 권고 또는 육성 트랙 등}
```

**톤 기준:** 권위 있는 이사회 의장의 선언문. 계산 결과가 아닌 판단의 이유를 말한다.
